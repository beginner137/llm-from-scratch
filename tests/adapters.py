...
def run_swiglu(
    ...
    from llm_lab.model.layers import SwiGLU
    swiglu = SwiGLU(d_model, d_ff, device=in_features.device,
                    dtype=in_features.dtype)
    swiglu.load_state_dict({
        "w1_weight.weight": w1_weight,
...
def run_scaled_dot_product_attention(
    ...
    """
    from llm_lab.model.layers import scaled_dot_product_attention
...
def run_multihead_self_attention(
    ...
    multi_head_attn = MultiHeadAttention(
        d_model, num_heads, device=in_features.device, dtype=in_features.dtype, with_rope=False)
...
def run_rope(
    ...
    """
    from llm_lab.model.layers import RotaryPositionalEmbedding
...
def run_transformer_lm(
    ...
    for layer_idx in range(num_layers):
        ...
        state_dict.update({
            f"{local_prefix}.ln1.weight": weights[f"{ref_prefix}.ln1.weight"],
            f"{local_prefix}.attn.q_proj_weight.weight": weights[f"{ref_prefix}.attn.q_proj.weight"],
            f"{local_prefix}.attn.k_proj_weight.weight": weights[f"{ref_prefix}.attn.k_proj.weight"],
            f"{local_prefix}.attn.v_proj_weight.weight": weights[f"{ref_prefix}.attn.v_proj.weight"],
            f"{local_prefix}.attn.o_proj_weight.weight": weights[f"{ref_prefix}.attn.output_proj.weight"],
            f"{local_prefix}.ln2.weight": weights[f"{ref_prefix}.ln2.weight"],
            f"{local_prefix}.ffn.w1_weight.weight": weights[f"{ref_prefix}.ffn.w1.weight"],
            f"{local_prefix}.ffn.w2_weight.weight": weights[f"{ref_prefix}.ffn.w2.weight"],
            f"{local_prefix}.ffn.w3_weight.weight": weights[f"{ref_prefix}.ffn.w3.weight"],
        })
...
def run_get_batch(
    ...
    """
    from llm_lab.training.loop import get_batch
...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    """
    from llm_lab.training.optimizer import gradient_clipping
...
def get_adamw_cls() -> Any:
    ...
    """
    from llm_lab.training.optimizer import AdamW
    return AdamW
...
def run_get_lr_cosine_schedule(
    ...
    """
    from llm_lab.training.scheduler import lr_cosine_schedule
...
def run_save_checkpoint(
    ...
    """
    from llm_lab.training.checkpoint import save_checkpoint
...
def run_load_checkpoint(
    ...
    """
    from llm_lab.training.checkpoint import load_checkpoint
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    # from llm_lab.tokenizer.bpe_standard import train_bpe
    from llm_lab.tokenizer.bpe_optimized import train_bpe
